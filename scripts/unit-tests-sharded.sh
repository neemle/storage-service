...
report_path = Path("scripts/tmp/unit-shards/coverage.txt")
...
region_cover = float(parts[3].strip("%"))
func_cover = float(parts[6].strip("%"))
line_cover = float(parts[9].strip("%"))
...
if branches_total == 0:
    branch_cover = 100.0
elif branches_cover.endswith("%"):
    branch_cover = float(branches_cover.strip("%"))
else:
    raise SystemExit(f"unexpected branch coverage value: {branches_cover}")

min_lines = float(os.environ.get("NSS_UNIT_COVERAGE_LINES", os.environ.get("UNIT_COVERAGE_LINES", "100")))
min_funcs = float(
    os.environ.get("NSS_UNIT_COVERAGE_FUNCTIONS", os.environ.get("UNIT_COVERAGE_FUNCTIONS", "100"))
)
min_regions = float(
    os.environ.get("NSS_UNIT_COVERAGE_REGIONS", os.environ.get("UNIT_COVERAGE_REGIONS", "100"))
)
min_branches = float(
...
if region_cover < min_regions:
    ...
    failed = True
if branch_cover < min_branches:
    print(f"Branch coverage {branch_cover:.2f}% below {min_branches:.2f}%")
